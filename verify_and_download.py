...
IDS = [
    ...
    "1575311373937-040b8e1fd5b6", "1616469829941-c7200edec809", "1610945415295-d9bbf067e59c", "1516035069371-29a1b244cc32",
    "1593642632823-8f785ba67e45", "1537498425277-c283d32ef9db", "1593642634367-d91a135587b5", "1527814050087-3793815479db",
    "1545454675-3531b543be5d", "1512790182412-b19e6d61b397", "1542751110-97427bbecf20", "1512499617640-c74ae3a79d37",
    ...
    "1517502884422-41eaead166d4", "1563203369-26f2e4a5ccf7", "1531297484001-80022131f5a1", "1484704849700-f032a568e944",
    "1511467687858-23d96c32e4ae", "1587614382346-4ec70e388b28", "1519389950473-47ba0277781c", "1515378791036-0648a3ef77b2",
    "1524678606370-a47ad25cb82a", "1538481199705-c710c4e965fc", "1567581935884-3349723552ca", "1583394838336-acd977736f90",
    "1563770660941-20978e870e26", "1580894732444-8ecded7900cd", "1550745165-9bc0b252726f", "1535223289827-42f1e9919769",
    "1550009158-9ebf69173e03", "1512446816042-444d641267d4", "1580927752452-89d86da3fa0a", "1563770660942-20978e870e27",
    "1526738549149-8e07eca6c147", "1546054454-aa26e2b734c7", "1567581935885-3349723552cb", "1517336714732-489689fd1ca9"
]
...
def main():
    assert len(IDS) == len(TARGET_FILES), f"Mismatch: {len(IDS)} IDs for {len(TARGET_FILES)} targets"
    assert len(IDS) == len(set(IDS)), f"Duplicate IDs present in list! Total: {len(IDS)}, Unique: {len(set(IDS))}"
    
    base_dir = r"c:\Users\infot\Contacts\OneDrive\Desktop\PDI\electric-shop\assets\images"
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}

    print(f"VERIFIED: 80 100% UNIQUE IDs allocated to 80 target files.")
    ...
    for rel_path, photo_id in zip(TARGET_FILES, IDS):
        ...
        try:
            ...
            # Check content hash to guarantee zero duplicate image data downloaded
            file_hash = hashlib.md5(content).hexdigest()
            assert file_hash not in downloaded_hashes, f"Duplicate image content detected for {rel_path}"
            ...
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
            success_count += 1
            print(f"[OK] Saved unique photo ({file_hash[:8]}) -> {rel_path}")
    ...
    assert success_count == len(TARGET_FILES), "Some images failed to download!"
